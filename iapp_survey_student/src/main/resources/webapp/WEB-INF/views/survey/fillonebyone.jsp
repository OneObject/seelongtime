		...
		<div class="survuy exam-box">
			...
			<c:forEach var="question" items="${list }" varStatus="index">
				<div id="con_${question.id}" data-id="${question.id }" style="${index.index != 0 ? 'display:none;' : ''}">
					<div>
						<input type="hidden" id="num_${question.id }" data-hasjump="${question.hasjump }" data-type="${question.type }"  data-must="${question.must }" value="${index.index }" data-max="${question.maxChooseNum }" data-min="${question.minChooseNum }"  autocomplete="off">
						<input type="hidden" id="q_index_${index.index }" value="${question.id }"  autocomplete="off"/>
						<!-- 单选start -->
						...
						<c:if test="${question.type == 4 }">
							<div class="questions"   id="${question.id }_${question.type}"  data-id="${question.id}"  data-type="${question.type }"  data-must="${question.must }">
								...
								<div class="star">
									<input type="hidden" id="${question.id }" value="" autocomplete="off">
									<input type="hidden" id="score_${question.id }" value="" autocomplete="off">
									<ul>
				...
				<div class="exam-btn-box">
					...
					<div class="fr">
						<input type="checkbox" id="allowAnonymous" name="allowAnonymous" value="1" style="margin-top: 40px;"><label for="allowAnonymous" style="font-size:16px;">&nbsp;<fmt:message key="fillonbyone.anonymous" bundle="${bundle}"/></label>
					</div>
					</c:if>
					</c:if>
					<c:if test="${index.index != size-1 }">
			        ...
			        success: function(code) {
			            ...
			            if(code==1){
			            		layer.msg('<fmt:message key="fillonbyone.submit_success" bundle="${bundle}"/>',{time:2000},function(){
				            		location.href="${basepath}/survey/prompt.xhtml?id="+"${entity.id}";
			            		});
				            return;
			            }
			            if(code==-2){
