...
<body id="activity-detail">
	...
	<div class="page-content">
		...
		<c:choose>
			<c:when test="${info.apply==1}"><%--需报名 --%>
				<c:if test="${empty record }">
					<div class="alert alert-info"><strong>请先报名</strong></div>
				</c:if>
				<c:if test="${not  empty record }">
			    	...
			    	<span class="btnGreen" id="baoming" onclick="signindialog()">我要签到</span>
			    	</div>
			    	</c:if>
		    	</c:if>
		    
			</c:when>
			<c:when test="${info.apply==0}"><%--无报名 --%>
				<c:if test="${not  empty record }">
			    	<c:if test="${record.isSignin eq 1 }">
			    	<div class="alert alert-success">
				        <strong>已签到</strong> 欢迎参加
              ...
              <p>${info.address }</p> --%>
              <div id="activityInfo" ${record.isSignin eq 1?'':'style="display:none;"' }>
	              <p><strong>培训时间</strong></p>
			  ...
			  html.push('<div style="width:100%;height:130px;">');
				...
				easyDialog.open({
				  ...
				  function btnFn(){
					  ...
					  $.post("${basepath}/protal/ixin/banduser/save.xhtml",{"id":"${openId}","mobile":userMobile,"name":userName,"company":userCompany},function(){
						  $.get("${basepath}/protal/activity/apply.xhtml", { activityId: "${info.id}", openId: "${openId}" },
			  				   function(data){
			  				     if(data == 0) {
			  				    	 opendialog('报名成功');
			  				    	 $("#baoming").remove();
			  			              $(".text").html('<span class="btnGreen" id="qiandao" onclick="signindialog()">我要签到</span>');
			  				     } else if(data == 1) {
	...
	var signin = function() {
		...
		if(code == "${info.signinCode}") {
			$.get("${basepath}/protal/activity/signin.xhtml", { activityId: "${info.id}", openId: "${openId}", code:code,baseuid:"${baseuid}"},
					   function(data){
					     if(data) {
					    	 opendialog('签到成功');
					    	 $("#baoming").remove();
