package com.longtime.common.filter;

import com.longtime.app.config.Constants;

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
import javax.servlet.Filter;
import javax.servlet.FilterChain;
import javax.servlet.FilterConfig;
import javax.servlet.ServletException;
import javax.servlet.ServletRequest;
import javax.servlet.ServletResponse;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import com.longtime.ajy.support.HttpServletHelper;
import com.longtime.common.utils.LoginSessionHelper;

public class LocalLoginFilter implements Filter {
	Logger logger = LoggerFactory.getLogger(LocalLoginFilter.class);

	private static final String REDIRECT_URL = "/mgr/login.xhtml";

	public List<String> excludes = new ArrayList<String>();

	@Override
	public void init(FilterConfig filterConfig) throws ServletException {
		logger.debug("LoginFilter init~~~~~~~~~~~~");
		
		
		
		Constants.WEBAPP_LOCAL_DIR = filterConfig.getServletContext().getRealPath("/");

		String temp = filterConfig.getInitParameter("excludes");
		
		if (temp != null) {
			String[] url = temp.split(",");
			for (int i = 0; url != null && i < url.length; i++) {
				excludes.add(url[i]);
			}
		}
	}

	@Override
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException,
			ServletException {
		logger.debug("LoginFilter do~~~~~~~~~~~~");
		HttpServletRequest req = (HttpServletRequest) request;
		HttpServletResponse resp = (HttpServletResponse) response;

		
		if(handleExcludeURL(req, resp)){
			chain.doFilter(request, response);
			return;
		}
		
		if (LoginSessionHelper.isLogin(req)) {
			chain.doFilter(request, response);
		} else {
			
			HttpServletHelper.setCallBackUrl(req);
			String redirect = String.format("%s%s", req.getContextPath() ,REDIRECT_URL);
			logger.debug("redirect=[{}]",redirect);
			resp.sendRedirect(redirect);
			return;
		}
	}

	@Override
	public void destroy() {

	}

	public boolean handleExcludeURL(HttpServletRequest request, HttpServletResponse response) {

		if (excludes == null || excludes.isEmpty()) {
			return false;
		}

		String url = request.getServletPath();
		for (String pattern : excludes) {
			Pattern p = Pattern.compile("^" + pattern);
			Matcher m = p.matcher(url);
			if (m.find()) {
				return true;
			}
		}

		return false;
	}

}
