...
<style type="text/css">
        	*{padding:0px;margin:0px;}
			...
			.head .headR ul{padding:10px 10px 10px 68px;box-sizing:border-box;min-width:400px;height:82px;position:absolute;left:0px;top:0px;border-radius: 15px;background:#3dac2e  url(${basepath}/static/wall/enn/img/lb.png) 18px center no-repeat; background-size:auto 40px;}
        	...
        	.msgBox .msg{width:750px;margin:10px 25px 10px 5px;font-size:32px;color:#fff;line-height: 44px;word-break:break-all;overflow-x: hidden;}
...
<section class="wallBox">
	<div class="head">
 				<div class="headL">
 					<img src="${basepath}/static/wall/enn/img/h1.png"  />
 				</div>
 				<div class="headC">
 					<img src="${basepath}/static/wall/enn/img/logo.png"  />
 				</div>
 				<div class="headR">
 					<div class="arr"></div>
 					<ul>
 						<li>关注微信公众号“新奥E学”</li>
 						<li>发送<label>“大学生”</label>获取上墙指南</li>
 					</ul>
 				</div>
 	</div>
							...
							<div class="msgBox" id="${comment.id}">
			 					<div class="imgBox">
			                       ...
			                       <c:if test="${not empty comment.headimgurl }">
			                       		<img src="${comment.headimgurl }" class="item_head"> 
			                       </c:if>
			 					</div>
			 					<div class="msg">
...
function changeBgColor(){
	...
	for(var i = 0; i < temp.length; i++){
		temp2 = jq(temp[i]);
		if(i % 2 === 0){
			temp2.find(".msgBody").attr("class","msgBody colorA");
			temp2.find(".msgBody > span").attr("class","a");
			}else{ 
				temp2.find(".msgBody").attr("class","msgBody colorB");
				temp2.find(".msgBody > span").attr("class","b");
				}
		}
}
</script>
