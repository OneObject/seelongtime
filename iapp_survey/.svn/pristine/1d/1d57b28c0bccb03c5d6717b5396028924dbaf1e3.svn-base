...
@Component("syncFromWxCpTask")
public class SyncFromWxCpTask {

	private static Logger logger = LoggerFactory.getLogger(SyncFromWxCpTask.class);
	...
	public void execute(){
		logger.info("SyncFromWxCpTask start");
		...
		for(TokenInfo token : list){
			try {
				doSyncFromWxCp(token);
			} catch (Exception e) {
				logger.error(String.format("sync from wxcp due to error,the token is [%s]", token),e);
			}
		}
		
		logger.info("SyncFromWxCpTask success");
	}
