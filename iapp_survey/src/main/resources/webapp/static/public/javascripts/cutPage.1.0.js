 ...
 * where_id：显示位置的id
 * 需要先在页面上面写一个容器<ul class="pagination" id="tab2cutpage"></ul>
...
function sallon_write_html(){
	...
	if(countPage <=10){
		//总页数小于10页
		if(countPage == 0){
			html.push('<li><a href=\'javascript:'+funtionName+'(1)\'>1</a></li>');
		}else{
...
function sallon_add_class(){
	//alert(currentPage+"333333333333333333");
	if(currentPage<=1){
...
function sallon_current_class(){
	//alert(current_page+"333333333333333333");
	$("#"+whereId).find("li").each(function(){
	...
	$("#"+whereId).find("li:nth-child("+(current_page+1)+")").addClass("active");
