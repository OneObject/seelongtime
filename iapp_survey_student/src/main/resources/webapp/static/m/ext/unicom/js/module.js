...
var simpleRichEditor = (function () {
    ...
    showProgress =  function () {//显示上传进度
        ...
        html =
            ...
            + '<dt ><label id="progress-label">0</label>%</dt>'
            + '<dd>'
    ...
    /* 上传图片功能 */
    createUploader = function (browseBtn, container, handlerUrl, paramsMap) {//browseBtn：定义上传按钮，container：：定义上传按钮的父容器，handlerUrl：定义后台处理程序的地址
    ...
    uploadCallback = function (type, up, temp, res) {//temp可能为files,file,err
        switch (type) {
            ...
            case 'FileUploaded':
                ...
                insertImage(JSON.parse(res.response).result);  //后台必须在上传成功时，返回一个包含图片URI的JSON格式的数据
                break;
            case 'UploadComplete':
                console.log('暂无操作！');
                break;
