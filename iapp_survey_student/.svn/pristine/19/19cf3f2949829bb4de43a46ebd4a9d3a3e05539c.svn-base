...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item imgs-item">';
		itemTemp +='<img alt="图片预览" src="'+basepath+'/static/m/new/img/pic.png" style="width:80px;height:80px;" id="pic_${fileID}">';
		...
		itemTemp += '</div>';
		itemTemp +='</div>';
		itemTemp += '<em class="delfilebtn btn btn-danger"><i class="fa fa-times"></i></em>';
		itemTemp +='</div>';
		itemTemp +='</div>';
	...
	this.each(function(){
		...
		var inputStr = '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="uploadify-button img-item-add"'; 
		inputStr += 'style="display:block;overflow:hidden;position:relative;">';
//		inputStr += option.buttonText;
		inputStr += '<i class="fa fa-plus"></i>';
	
		inputStr += '<input id="select_btn_'+instanceNumber+'" class="selectbtn" name="pic" style="opacity:0;position:absolute;width:80px;height:80px;margin-left:-80px;z-index:10;" type="file" name="fileselect[]"';
...
//		inputStr += '"';
        inputStr += '/>';
    	inputStr += '</a>';
		var uploadFileListStr = '<div id="file_upload_'+instanceNumber+'-queue" class="uploadify-queue preview-area" style="float:left;list-style:none;"></div>';
	  ...
	  var fileObj = {
				  ...
				  for(var i=0,len=files.length;i<len;i++){
				  	var thisFile = files[i];
				  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
				  			...
				  			J.showToast('文件'+thisFile.name+'大小超出限制！',"toast",1500);
				  			fileObj.fileInput.val('');//删除文件选择框里的该文件
				  			continue;
				  		}
						if($.inArray(thisFile.name.split('.').pop(),typeArray)>=0){
                           ...
                           //arr.push(thisFile);
							...
							J.showToast('文件'+thisFile.name+'类型不允许！',"toast",1500);
		  ...
		  onSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					...
					if(option.auto){
						...
						//如果配置非自动上传，绑定上传事件
					 	$html.find('.uploadbtn').on('tap',(function(file){
					 			return function(){fileObj.funUploadFile(file);}
					 		})(file));
					}
					//为删除文件按钮绑定删除文件事件
			 		$html.find('.delfilebtn').on('tap',(function(file){
		  ...
		  funDealFiles: function(files) {
			  var fileCount = countIndex++; // _this.find('.uploadify-queue .uploadify-queue-item').length;//队列中已经有的文件个数
			  for(var i=0,len=files.length;i<len;i++){
