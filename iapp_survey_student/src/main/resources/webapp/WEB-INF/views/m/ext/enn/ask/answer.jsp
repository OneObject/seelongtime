...
<body class="bg-color-gray">
	...
	<c:choose>
		...
		<%-- 1.1登陆人和提问问题的作者相同    开始 --%>
		<c:when test="${entity.author_id eq user.id }">
			<section class="wrapper-static-chat margin-top-50 margin-bottom-71"
				id="chat-wrapper">
				<%--
            	<div class="system-info">4月27日 早上 10:12</div>
         	--%>
				<%-- 问题 --%>
				<div class="me">
					<div class="chat-box">
						...
						<div class="content-line-1">${entity.supplement }</div>
				...
				<div class="other">
					<div class="chat-box">
						...
						<div class="name">
							${answer.author_name }
							<%--
							<c:if test="${answer.isexpert eq 1 }">
								<span class="hat"></span>
							</c:if>
							 --%>
						</div>
			...
			<section class="block-fixed-func-chat add-block-fixed-func-chat"  id="block-fixed-func-chat">
						<div class="wrapper-func add-wrapper-func" id="wrapper-func">
							<span class="plus img" id="insert-image"><i
								class="fa fa-picture-o"></i></span>
							<!--<div>
								<input type="text" id="input-text" placeholder="请输入内容" />
							</div>-->
							<div class="add-wrapper-textarea"><textarea class="replace-input" id="input-text" placeholder="请输入内容"></textarea></div>
			...
			<!-- <section class="block-fixed-func-chat">
				<div class="wrapper-func" id="wrapper-func">
					<span class="plus img" id="insert-image"><i
						class="fa fa-picture-o"></i></span>
					<div class="add-wrapper-textarea">
						<input type="text" id="input-text" placeholder="请输入内容" />
					</div>
					<span class="send" id="btn-send">发送</span>
				</div>
		...
		<c:otherwise>
			<c:choose>
			<%-- 1.2登陆人和提问问题的作者不相同   开始 --%>
				<c:when test="${not empty answer and answer.author_id ne user.id }">
					<section class="wrapper-static-chat margin-top-50 margin-bottom-71"
						id="chat-wrapper">
						<%-- 问题 --%>
						<div class="other">
							<div class="chat-box">
								...
								<div class="name">${entity.anonymous eq 0 ? entity.author_name :'匿名' }</div>
								<div class="content-line-1">${entity.supplement }</div>
								<label class="arrow set-bg-color"></label>
							</div>
						</div>
						...
						<c:forEach var="item" items="${list }">
							...
							<c:when test="${entity.author_id eq item.uid and entity.anonymous eq '1' }">匿名</c:when><c:otherwise>${item.uname}</c:otherwise>
										...
										<%--
										<c:if
											test="${item.uid eq answer.id and answer.isexpert eq 1 }">
											<span class="hat"></span>
										</c:if>
										 --%>
									</div>
				...
				<%-- 1.2登陆人和提问问题的作者不相同  且  答案存在  结束  --%>
				<c:otherwise>
				
				<%-- 1.3 既不是登陆人回答也不是登陆人提问 --%>
					...
					<section class="wrapper-static-chat margin-top-50 margin-bottom-71"
						...
						<c:choose>
							...
							<%--  
											<img src="${basepath}/user/avatar/${item.uid}.xhtml?style=80x80&color=gray&r=<%=random%>" />
							--%>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			if ($('#block-fixed-func-chat').length) {
				$('#block-fixed-func-chat')[0].ontouchmove = function () {
					return false;
				};
			}
			...
			setTimeout(function () {
				$window.scrollTop($chatWrapper.prop('scrollHeight') - windowSize[1] + 121);
			}, 300);
		...
		function createOtherChatBox(initMap, hasHat) {
			...
			case 'text':
				...
				html = '<div class="other">'
						+ '<div class="chat-box">'
						+ '<span class="image"><img src="'+ initMap.iconUrl +'" /></span>'
						+ '<div class="name">' + initMap.name +'00'+ hatHtml
