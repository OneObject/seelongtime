...
<c:forEach var="item" items="${list }" varStatus="v">
	...
	<div class="block-friend-list" style="display: ${map[item.gid]=='open'  ?'block':'none' };">
		<c:forEach var="entity" items="${item.list }" >
			<div class="someone" data-name="${entity.name }" data-id="${entity.id }" data-href="abc" onclick="toUserDetail('${entity.id}');">
				<span class="image">
					<img src="${basepath}/user/avatar/${entity.id}.xhtml?style=80x80&color=gray" />
				</span> <label><i
