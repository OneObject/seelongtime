...
<head lang="en">
    ...
    <style type="text/css">
        ...
        *{padding:0px;margin:0px;}
        	...
        	.head{padding-bottom:10px;width:1024px;}
        	.head .headL{width:400px;height:84px;margin:34px 10px 0px 48px;float:left; }
        	.head .headL img{height:84px;display:block;}
        	.head .headC{width:118px;height:108px;float:right;margin-right:20px;margin-top:10px;display:block;}
        	.head .headC img{height:108px;width:118px;display:block;}
			.content{padding:30px 20px 60px 45px;width:100%;box-sizing:border-box;cursor:pointer;}
			.content > div.highCharts{padding:2px;background: #FFF383;width:100%;box-sizing:border-box;}
			#wrapper{width:100%;}
			#table-box{width:100%;}
			#bg{background:#fff;width:100%;padding:40px 0px 45px 0px;}
			#zhengchang{float:right;margin-right:20px;}
 			...
 			<div class="head">
 				<div class="headL">
                    ...
                    <c:if test="${empty param.type }">
 					    <img src="${basepath}/static/img/vote_enn_20151124.png"/>
                    </c:if>

                    <c:if test="${param.type eq 'mmkh' }">
                        <img src="${basepath}/static/img/mmkh.png"/>
                    </c:if>

                    <c:if test="${param.type eq 'czsh' }">
                        <img src="${basepath}/static/img/czsh.png"/>
                    </c:if>


 				</div>
 				<div class="headC">
 					<img src="${basepath}/static/img/vote_enn_logo.png"  />
 				</div>
 			</div>
...
if(ie){
    ...
    function loadData(){
        ...
        $.ajax({
     	...
     	if(question.title){
     		...
     		document.title=question.title + "-实时投票结果";
     	}
    ...
    function dabao(poll){//将处理后的数据打包
       ...
       // console.log(serie.length+"系列的个数");
        ...
        serie[1].visible=false;//默认将第二个不显示
        return serie;
    }
    function calculate(poll) {
        ...
        for(var i =0;i<poll.length;i++){
         BFB.push(parseInt(parseInt(poll[i])/parseInt(all)*100));
        }
        return BFB;//输出
    }
    function initchats(){
