         ...
         <div class="row">
              <div class="col-md-12">
               ...
               <div class="portlet box blue">
                  ...
                  <div class="portlet-body" style="overflow: hidden;padding-bottom: 200px;">
                      <div class="row">
                            <span class="btn btn-success table_add"  onclick="insertEntity();">
                               <i class="glyphicon glyphicon-plus"></i>新建活动
                            </span>
                      </div>
                      <%-- 表格内容 --%>
...
page(1);
function page(i){
	$('#table_list').load("${basepath}/mgr/ixinnofilter/activitylist.xhtml?currPage="+i);
}
function insertEntity(){
	location = "${basepath}/mgr/activity/step1.xhtml";
}
...
var btnFn = function(){
	$.ajax({
		type: "POST",
		url:  basepath+"/mgr/activity/deleteactivity.xhtml",
		data: {"id":m},
...
function delActivity(id){
	m = id;
	var html = '<div style="width:350px;height:70px;text-align:center;"><h3>您确定要删除该活动么？</h3></div>';
	easyDialog.open({
	    container : {
	    	header : 'ajy温馨提示',
	    	content : html,
