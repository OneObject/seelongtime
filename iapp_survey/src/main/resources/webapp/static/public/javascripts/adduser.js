...
function usernameValidate(userId, conSelf){
	...
	if(true){
		...
		DWRService.uniqueValidate2($("#domain").val(),"user","username", value, userId, conSelf, true, function (ret){
			s = ret;
		});
...
function checkUserName(){
	  ...
	  if (userName.length==0){
		  ...
		  $.ajax({
			  type:"POST",
			  url:basepath+"/base/admin/user/uniqueValidate.json",
...
function mobileValidate(userId, conSelf){
	...
	if(mobileFormatCheck(value)){
		dwr.engine.setAsync(false);
		DWRService.uniqueValidate2($("#domain").val(),"user", "mobile", value, userId, conSelf, true, function (ret){
			s = ret;
		});
		dwr.engine.setAsync(true);
		if(s){
...
function emailValidate(userId, conSelf){
	...
	if(emailFormatCheck(value)){
		dwr.engine.setAsync(false);
		DWRService.uniqueValidate2($("#domain").val(),"user", "email", value, userId, conSelf, true, function (ret){
			s = ret;
		});
		dwr.engine.setAsync(true);
		if(s){
...
function submitSuccessBak(ret){
	if(ret.flag){
		myAutoHideDialog(ret.message, "success", function (){location.href = basepath+"/base/admin/user/usermanage.xhtml";})
	} else{
...
function updateUserValidate(){
	...
	if (userName.length==0){
		...
		if(checkPassword()&&nameValidate()&&emailValidate(userId, true) && certificateCodeValidate() && mobileValidate(userId,true) && orgIdValidate() && userNameIsRepart){
			...
			//处理角色修改
			if(isCheckCategory==true){
				return true;
			}
			
			var oldCategory = $.trim($("#oldCategory").val()); //老的角色
			...
			if(category!=oldCategory){
				...
				if(oldCategory=='1' || oldCategory=='0'){
					
					if(confirm("修改用户角色，将清除当前用户的所有辅导关系（指导人-学员 或 学员-指导人）？")){
