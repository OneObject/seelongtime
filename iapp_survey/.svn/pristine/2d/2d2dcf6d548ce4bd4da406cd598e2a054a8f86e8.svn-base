...
public class HttpServletHelper {
    ...
    public static String getCurrentUrl(HttpServletRequest request){
    	//http://localhost:8888/iapp/mgr/index.xhtml
    	String s1 = Joiner.on("?").skipNulls().join(request.getRequestURL().toString(), request.getQueryString());
    	
        return  s1;// eg. http://xxx:xx/iapp/study/me.do?a=b
	...
	public static String getAbsolutePath(HttpServletRequest request,
			String relativePath) {
		return String.format("%s/%s", getRootPath(request),relativePath);
	}
