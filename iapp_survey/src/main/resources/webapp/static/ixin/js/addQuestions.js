...
function addOption(){
	...
	if(type == 6){ //填空题选项特殊
		...
		var nextIdNumber = $(".con_completion_item").last().data("num")+1;
		var optionsInnerHtml = new Array();
		optionsInnerHtml.push('<div id="con_completion_item_'+nextIdNumber+'" class="con_completion_item" data-num="'+nextIdNumber+'">');
		optionsInnerHtml.push('<div class="col-md-8 form-group "  id="con_title'+nextIdNumber+'">');
		optionsInnerHtml.push('<input type="text" class="form-control" name="title'+nextIdNumber+'" id="title'+nextIdNumber+'" placeholder="请输入题干">');
		optionsInnerHtml.push('</div>');
		optionsInnerHtml.push('<div class="col-md-2 form-group"  id="con_inputBoxSize'+nextIdNumber+'">');
		optionsInnerHtml.push('<input type="text" class="form-control" name="inputBoxSize'+nextIdNumber+'" id="inputBoxSize'+nextIdNumber+'" placeholder="15">');
		optionsInnerHtml.push('</div>');
		optionsInnerHtml.push('	<span class=" col-md-2 form-group">');
		optionsInnerHtml.push('		<button class="btn red" type="button"  onclick="removeCompletion(\''+nextIdNumber+'\')"><i class="fa fa-times" style="font-size: 18px;"></i></button>');
		optionsInnerHtml.push('	</span>');
		optionsInnerHtml.push('</div>');
		$('#vote_completion').append(optionsInnerHtml.join(''));
	} else {	//非填空题
		count = count+1;
...
function removeCompletion(num){
	var optionId = $("#optionId"+num).val();
	var currentObj = $("#con_completion_item_"+num);
	if(typeof(optionId)!="undefined"&&$.trim(optionId)!=null && $.trim(optionId)!=""){
		$.post(basepath+"/mgr/vote/deletevoteoption.xhtml",{"optionId":optionId},function(){
			currentObj.remove();
		});
...
function backToList(surveyOrvote){
	window.location.href = basepath + "/mgr/vote/index.xhtml";
}
...
function getRequestOptions(){
	...
	if(questionType == 6){
		var size = $(".con_completion_item").length;
		for(var i=0;i<size;i++){
			var item = $(".con_completion_item")[i];
			var num = $(item).data("num");
			var optionId = $("#optionId"+num).val();
			var title = $("#title"+num).val();
			var inputBoxSize = $("#inputBoxSize"+num).val();
			if(isBlank(inputBoxSize)){
				inputBoxSize = 15;
			}
			var optionObj = new Object();
			optionObj.id = optionId;
			optionObj.title = title;
			optionObj.inputBoxSize = inputBoxSize;
...
function v_completion_items(){
	var size = $(".con_completion_item").length;
	for(var i=0;i<size;i++){
		var item = $(".con_completion_item")[i];
		var num = $(item).data("num");
		var titleId = "title"+num;
		var inputBoxSizeId = "inputBoxSize"+num;
		...
		value = $.trim(obj.val());
		if(isBlank(value)){
			removeError(inputBoxSizeId);
		} else {
			if(!intCheck(value)){
				alert("请输入5-15之间的正整数！");
				obj.focus();
				addError(inputBoxSizeId);
				return false;
...
function getCompletionTitle(){
	var item = $(".con_completion_item")[0];
	var num = $(item).data("num");
	var title = $("#title"+num).val();
	return title;
}
...
function resetForm(){
	...
	formInnerHtml.push('			<label class="radio-inline" style="margin-left:22px;">');
	...
	formInnerHtml.push('            <label class="radio-inline">');
	formInnerHtml.push('            <input type="radio" name="questionType" value="6">填空题</label>');
	...
	formInnerHtml.push('<input type="text" class="form-control"  name="minChooseNum" id="minChooseNum" placeholder="设置最少选择数，默认为0，代表无限制" onchange="v_minChooseNum();">');
	formInnerHtml.push('</div>');
	formInnerHtml.push('</div>');
	
	formInnerHtml.push('<div class="row" id="vote_completion" style="display: none;">');
	formInnerHtml.push('<div id="con_completion_item_1" class="con_completion_item" data-num="1">');
	formInnerHtml.push('<div class="col-md-8 form-group "  id="con_title1">');
	formInnerHtml.push('<label>题干</label>');
	formInnerHtml.push('<input type="text" class="form-control" name="title1" id="title1" placeholder="请输入题干">');
	formInnerHtml.push('</div>');
	formInnerHtml.push('<div class="col-md-4 form-group"  id="con_inputBoxSize1">');
	formInnerHtml.push('<label>填空输入框长度(字符数5-15)</label>');
	formInnerHtml.push('<input type="text" class="form-control" name="inputBoxSize1" id="inputBoxSize1" placeholder="15">');
	formInnerHtml.push('</div>');
	formInnerHtml.push('</div>');
	formInnerHtml.push('</div>');
	...
	formInnerHtml.push('					<input type="text" class="form-control" placeholder="1" value="1" disabled="disabled">');
	formInnerHtml.push('					<input type="hidden" class="form-control" name="minScore" id="minScore" value="1">');
	formInnerHtml.push('					<span class="help-block hidden" id="tip_minScore">1</span>');
	formInnerHtml.push('				</div>');
	...
	formInnerHtml.push('					<span class="help-block hidden" id="tip_maxScore">2-10</span>');
	formInnerHtml.push('				</div>');
...
function voteQuestionEcho(data){
	...
	var completionIsChecked=(data.type==6?"checked='checked'":"");
	...
	var isDisabled6 = data.type==6 ?"":"disabled=disabled";
	...
	voteFormInnerHtml.push('			<label class="radio-inline" style="margin-left:22px;">');
	...
	voteFormInnerHtml.push('             <label class="radio-inline">');
	voteFormInnerHtml.push('            <input type="radio" name="questionType" value="6" '+completionIsChecked+' '+isDisabled6+'>填空题</label> ');
	...
	if(data.type != 6){
		...
		voteFormInnerHtml.push('		<div class="form-group" style="display:none;">');
	}
	...
	if(data.type==2  ){
		...
		voteFormInnerHtml.push('<input type="text" class="form-control" value="'+data.minChooseNum+'" name="minChooseNum" id="minChooseNum" placeholder="设置最少选择数，默认为0，代表无限制" onchange="v_minChooseNum();">');
		voteFormInnerHtml.push('</div>');
		voteFormInnerHtml.push('</div>');
	}
	if(data.type == 6){
		voteFormInnerHtml.push('<div class="row" id="vote_completion">');
		for(var i=0;i<data.options.length;i++){
			var cnum = i+1;
			voteFormInnerHtml.push('<div id="con_completion_item_'+cnum+'" class="con_completion_item" data-num="'+cnum+'">');
			voteFormInnerHtml.push('<input type="hidden" id="optionId'+cnum+'" value="'+data.options[i].id+'"/> ')
			voteFormInnerHtml.push('<div class="col-md-8 form-group "  id="con_title'+cnum+'">');
			if(i==0){
				voteFormInnerHtml.push('<label>题干</label>');
			}
			voteFormInnerHtml.push('<input type="text" class="form-control" name="title'+cnum+'" id="title'+cnum+'" placeholder="请输入题干" value="'+data.options[i].title+'">');
			voteFormInnerHtml.push('</div>');
			if(i!=0){
				voteFormInnerHtml.push('<div class="col-md-2 form-group"  id="con_inputBoxSize'+cnum+'">');
			} else {
				voteFormInnerHtml.push('<div class="col-md-4 form-group"  id="con_inputBoxSize'+cnum+'">');
			}
			if(i == 0){
				voteFormInnerHtml.push('<label>填空输入框长度(字符数5-15)</label>');
			}
			voteFormInnerHtml.push('<input type="text" class="form-control" name="inputBoxSize'+cnum+'" id="inputBoxSize'+cnum+'" placeholder="15" value="'+data.options[i].inputBoxSize+'">');
			voteFormInnerHtml.push('</div>');
			if(i!=0){
				voteFormInnerHtml.push('	<span class=" col-md-2 form-group">');
				voteFormInnerHtml.push('		<button class="btn red" type="button"  onclick="removeCompletion(\''+cnum+'\')"><i class="fa fa-times" style="font-size: 18px;"></i></button>');
	...
	if(data.type == 1 || data.type == 2){
		...
		voteFormInnerHtml.push('					<input type="text" class="form-control" value="'+data.minScore+'" disabled="disabled">');
		voteFormInnerHtml.push('					<input type="hidden" class="form-control" name="minScore" id="minScore" value="'+data.minScore+'">');
		voteFormInnerHtml.push('					<span class="help-block hidden" id="tip_minScore">1</span>');
		voteFormInnerHtml.push('				</div>');
		...
		voteFormInnerHtml.push('					<span class="help-block hidden" id="tip_maxScore">2-10</span>');
		voteFormInnerHtml.push('				</div>');
