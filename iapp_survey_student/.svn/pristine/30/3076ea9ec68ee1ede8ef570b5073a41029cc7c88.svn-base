...
<%@page import="com.longtime.ajy.mweb.service.VideoService"%>
...
<%
    ...
    if(null!=video){
        ...
        if(isAudio){
            cover = request.getContextPath() + "/static/img/audio.png";
        }
...
<%
    boolean  isLocalResource = StringUtils.isNotBlank(type) && "local".equals(type);
    // 本地视频
    if(isLocalResource){
        if(!isMobile){
%>
...
<%
        ...
        String resrouceUrl = String.format("%s/jvideo/player/%s.m3u8",Constant.IVIDEO_SERVER,videoId);
        if(StringUtils.isNotBlank(mp4url)) {
            //播放地址替换
            resrouceUrl = mp4url.replace("121.41.116.92", "video.aijiayou.com");
        }

        if(!isAudio){
%>
            <video src="<%=resrouceUrl %>" controls  width="100%" height="100%" style="position:absolute;" poster="<%=cover%>"></video>
<%
        }else{
%>
            <audio controls="controls" style="width:100%">
                <source src="<%=resrouceUrl %>"/>
    　　     </audio>
<%
        }
    }
