                    ...
                    <div class="check-task-item">
                        <div class="item-top">
                            ...
                            <div class="name-time-score">
                                ...
                                <c:if test="${task.score>item.score}">
                                 <span class="nts-score">不通过</span>
                                </c:if>
                                 <c:if test="${task.score<=item.score}">
                                <span class="nts-score">通过</span>
                                </c:if>
                            </div>
                        </div>
                            ...
                            <figure data-count="6张图片">
                                <div>
                                  <c:forEach var="m" items="${item.attachments}">
                                   <img src="<%=Constant.IMG_SERVER_DOWNLOAD%>${m.url}-80x80" data-src="<%=Constant.IMG_SERVER_DOWNLOAD%>${m.url}-1024x1024" data-gid="${item.uid}" onload="preViewImg(this, event);">
                                   </c:forEach>
                                </div>
                            </figure>
                        </div>
                        <div class="item-footer">
                            <span onclick="showinput(1,'${item.id}','${item.uid}')" style="border: none;width: 100%;"><i class="fa fa-keyboard-o"></i>写评语</span>
                        </div>
                    </div>
                </li>
                </c:forEach>
