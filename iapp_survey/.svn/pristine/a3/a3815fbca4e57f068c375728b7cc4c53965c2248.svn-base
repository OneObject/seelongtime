...
function page(i, fieldId) {
	...
	var url = basepath+"/base/admin/resource/list.list";
...
function addResource(){
	window.location.href=basepath+"/base/admin/resource/step1.xhtml";
}
function update(id){
	window.location.href=basepath+"/base/admin/resource/step1.xhtml?id="+id;
}
function release(id,release){
	var tempFn = function (){
		$.ajax({
			type:"POST",
	...
	var html="<div style='width:300px;height:50px;text-align:center;'>"+content+"</div>";
		easyDialog.open({
			container : {
			    header : '提示',
			    content : html,
...
function deleteEntity(id){
	var tempFn = function (){
		$.ajax({
			type:"POST",
	   	 	url:basepath+"/base/admin/resource/delete.json",
	   	 	data:{id:id},
	   	 	success:function(data){
	   	 		page(1);
	   	 	}
