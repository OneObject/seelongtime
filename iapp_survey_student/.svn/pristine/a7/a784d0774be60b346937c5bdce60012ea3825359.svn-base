...
J.Page = (function($){
    ...
    var loadSectionTpl = function(hash,callback){
        ...
        var query;
        var appendTpl = true;
        //{hash: "#qiandao?apd=111", tag: "#qiandao", query: "apd=111", param: Object}   hash:封装后的链接
...
J.Router = (function($){
    ...
    var _targetHandler = function(){
        ...
        switch(target){
            case 'section' :
                ...
                break;
            case 'article' :
                _showArticle(href,_this,callback);
                break;
            case 'menu' :
                _toggleMenu(href);
                break;
     ...
     * @param el 当前锚点
     */
    var _showArticle = function(href,el,callback){
        var article = $(href);
 ...
 * @param duration 持续时间，为0则不自动关闭
 */
J.showWarn = function(text,type,duration){
    type = type || 'toast';
    J.Warnalert.show(type,text,duration);
}
/**
 * 关闭消息提示
 */
 ...
 * 自定义弹出警告提示
 * */
J.Warnalert = (function($){
    var WARN_DURATION = 2000;
    //定义模板
    var TEMPLATE = {
        toast : '<a href="#">{value}</a>',
        success : '<a href="#"><i class="icon checkmark-circle"></i>{value}</a>',
    ...
    var warn_type = 'warn',_warn,timer;
    var _init = function(){
        //全局只有一个实例
        $('body').append('<div id="jingle_warn"></div>');
        _warn = $('#jingle_warn');
        _subscribeCloseTag();
    }
     ...
     */
    var hide = function(){
        J.anim(_warn,'scaleOut',function(){
            _warn.hide();
            _warn.empty();
        });
    }
