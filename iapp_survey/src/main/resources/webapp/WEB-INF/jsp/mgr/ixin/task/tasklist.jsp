...
<table class="table table-striped table-bordered table-hover table-full-width" id="sample_2">
	...
	<tr>       
               ...
               <td>
              ${ltfun:html2Text(item.taskrequire,25)}
               </td>
               <td>${item.score }</td>
               ...
               <a href="javascript:exportResult('${item.id}');">结果导出</a>
               </td>
		</tr>
	</c:forEach>
...
<script type="text/javascript">
    ...
    function exportResult(id){
    		$.ajax({
    		    type:"POST",
    		    url:"${basepath}/mgr/task/exportresult.json",
    		    data:{"id":id},
    		    success:function(data){
    		    		if(data == "fail"){
    		    			showMsgDialog("系统异常，请刷新后重新尝试。");
    		    		}else if(data == "empty"){
    		    			showMsgDialog("还没有人提交作业，请确认后再操作。");
    		    		}else if(data == "illegal"){
    						showMsgDialog("作业不存在或已被删除，请确认后操作。");
    					}else {
