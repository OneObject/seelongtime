        ...
        <div class="login-box">
        	<form action="${basepath}/m/h5/login.xhtml" method="post" id="loginForm">
	            <label>
	                ...
	                <input type="text" placeholder="账号" id="username" name="username" value="${loginForm.username }">
	            </label>
	            <label>
	                <i class="fa fa-unlock"></i>
	                <input type="password" placeholder="密码" id="password" name="password">
	            </label>
	            	<span class="errormsg" id="hasNull" style="display: none;">请输入账号/密码！</span>
	            <div id="error">
		            <c:if test="${not empty loginForm and loginForm.code !='success'}">
		               <c:choose>
		                	<c:when test="${loginForm.code=='empty'}"><span class="errormsg">请输入账号密码！</span></c:when>
		                	<c:when test="${loginForm.code=='pwdfial'}"><span class="errormsg">账号或密码错误！</span></c:when>
		                	<c:when test="${loginForm.code=='disable'}"><span class="errormsg">账号被禁用</span></c:when>
		                	...
		                	<c:when test="${loginForm.code=='unequal_domain'}"><span class="errormsg">账号在本公司不存在</span></c:when>
		               </c:choose>
	                </c:if>
	            </div>
	            <span class="btn-login" onclick="submit()">绑定</span> 
