...
//	var itemTemp = '<li id="${fileID}"><div class="uploadify-queue-item">';
//		itemTemp +='<div class="funjian-item">';
//		itemTemp +='<div class="imgbox"><img alt="图片预览" src="http://ajy-test.qiniudn.com/FuM7URiPeXma5cpd8GZUg6fIQCur" id="pic_${fileID}"></div>';
//		itemTemp += '<div class="detail-control">';
//		itemTemp += '<div class="itemdetail">';
//		itemTemp += '<div class="progress uploadify-progress">';
//		itemTemp += '<div class="progress-bar progress-bar-success uploadify-progress-bar" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 10%">';
//		itemTemp += '<span style="display:none;" id="progress_${fileID}" ></span>';
//		itemTemp += '</div>';
//		itemTemp += '</div>';
//		itemTemp += '<span class="fujian-name up_filename" id="name_${fileID}"></span>';
//		itemTemp +='</div>';
//		itemTemp += '<div class="fujian-control">';	
//		itemTemp += '<span class="delfilebtn btn btn-danger"><i class="fa fa-times"></i>&nbsp;移除</span>';
//		itemTemp += '</div>';
//		itemTemp +='</div>';
//		itemTemp +='</div>';
////		itemTemp +='<span class="uploadbtn">上传</span></div>';
//		itemTemp +='</div></li>';
	var itemTemp = '<div class="loadImgBox">';
		itemTemp += '<label class="num">0%</label>';
		itemTemp += '<img src="'+bgImgPath+'" />';
		itemTemp += '<div class="perBox">';
		itemTemp += '<span class="active per" data-width="44"></span>';
		itemTemp += '</div>';
		itemTemp += '</div>';
	var defaults = {
		...
		fileSizeLimit: 10000,//允许上传的文件大小，单位KB
	...
	this.each(function(){
		...
		var inputStr = '<input id="select_btn_'+instanceNumber+'" class="selectbtn" style="opacity:0;" type="file" name="fileselect[]"';
...
//		inputStr += '"';
		...
		inputStr += 'style="width:100%;height:50px;display:block;overflow:hidden;background-color:#eaeaea;border:1px solid #dadada;text-align:center;font-size:36px;">';
//		inputStr += option.buttonText;
		inputStr += '<i class="fa fa-plus"></i>';
		inputStr += '</a>';
		var uploadFileListStr = '<div><ul id="file_upload_'+instanceNumber+'-queue" class="uploadify-queue preview-area"></ul></div>';
...
//							alert('文件'+thisFile.name+'类型不允许！');
		  ...
		  onProgress: function(file, loaded, total) {
				...
				//bugong
				$(".loadImgBox .num").text(percent);
				$(".perBox .per").width(percent);
