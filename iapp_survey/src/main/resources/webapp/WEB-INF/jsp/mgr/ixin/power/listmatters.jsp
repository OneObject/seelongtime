...
<div class="portlet-body box" style="position: relative; height:500px;overflow-y:scroll;overflow-x:hidden;" >
<ul class="sucailist" style="width:700px;">
<c:set var="domain" value="<%=LoginSessionHelper.getCurrentDomain(request)%>"></c:set>
...
<div class="select-active hidexbox" onclick="chooseselect(this);" >
  <c:if test='${idsMap[info.id]}'> <i class="glyphicon glyphicon-ok" style="margin-top:80px;"></i></c:if>
 </div>
</li>
...
<script type="text/javascript">
  function chooseselect(thi){
	  if($(thi).parent("li").attr("class") == "one-a cactive" || $(thi).parent("li").attr("class") == "cactive one-a"||$(thi).parent("li").attr("class") == "one-a  cactive" || $(thi).parent("li").attr("class") == "cactive  one-a" ){
            ...
            $(thi).html('<i class="glyphicon glyphicon-ok" style="margin-top:80px;"></i>');
            onSave(thi);
		  }
