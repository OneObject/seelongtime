		...
		<div class="col-lg-12">
			<div class="portlet box green">
				<div class="portlet-title">
					<div class="caption">
						<i class="glyphicon glyphicon-plus"></i><span>活动修改</span>
					</div>
				</div>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="col-md-5" style="min-width: 400px;">
									<div class="preview">
										<div class="title">
											<a href="#" id="titleName">${reward.explain1.title }</a>
										</div>
										<div class="datetime">
										<c:if test="${not empty reward.startDate }">
										<d:formatDate currentTime="${reward.startDate}" format="yyyy-MM-dd HH:mm"/>
										<c:if test="${not empty reward.endDate}">
										-<d:formatDate currentTime="${reward.endDate}" format="yyyy-MM-dd HH:mm"/>
										</c:if>
										</c:if>
										<c:if test="${empty reward.startDate }">
										<c:if test="${not empty reward.endDate}">
										<d:formatDate currentTime="${reward.endDate}" format="yyyy-MM-dd HH:mm"/>
										</c:if>
										</c:if>
										...
										<div class="img" >
											<img src="<%=Constants.IMG_SERVER_DOWNLOAD%>${reward.explain1.picUrl }" id="imgForLook">
										</div>
										<div class="description">
											${reward.explain1.content }
										</div>
									</div>
							</div>
							<div class="col-md-6">
								<div class="edit">
									<div class="form">
											<form role="form" id="form1" action="${basepath }/mgr/scratchcard/modifyrewardfirststep.xhtml" method="post">
												<div class="form-body">
													<div class="form-group" id="con_title1">
														<label for="title1" class="control-label">活动标题<span class="required">*</span></label>
														<input type="email" class="form-control" id="title1" name="title1" value="${reward.explain1.title}">
													...
													<div class="form-group" id="con_picUrl1">
														...
														<div class="upload_box">
															<div class="upload_area">
																<input type="hidden" name="picUrl1" id="picUrl1" value="${reward.explain1.picUrl }">
																<input type="file" name="file_upload" id="file_upload" />
																<ul class="upload_file_box" style="display: none;" id="queue"></ul>
															</div>
															<p class="js_cover upload_preview" >
																<div style="display:" id="small_prev">
																	<span id="image_cover">
																	<img style="max-width:100px;max-height:200px;" src="<%=Constants.IMG_SERVER_DOWNLOAD%>${reward.explain1.picUrl }">
													...
													<div class="form-group" id="con_content1">
														...
														<textarea class="form-control" rows="3" id="content1" name="content1">${reward.explain1.content}</textarea>
														<span class="help-block">
														</span>
													</div>
													<div class="form-group" id="con_startDate">
														...
														<table>
															<tr>
																<td>从</td><td><input type="text" id="startDate" class="form-control" name="startDate"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',maxDate:'#F{$dp.$D(\'endDate\')}'})" class="form-control Wdate" value='<d:formatDate format="yyyy-MM-dd HH:mm" currentTime="${reward.startDate }"/>'/></td>
																<td>到</td><td><input type="text" id="endDate" class="form-control" name="endDate"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm', minDate:'#F{$dp.$D(\'startDate\')}'})" class="form-control Wdate" value='<d:formatDate format="yyyy-MM-dd HH:mm" currentTime="${reward.endDate }"/>'/></td>
															</tr>
														</table>
														<span class="help-block">
														</span>
													...
													<%-- 
													 <div class="form-group" id="con_apply">
                       								 <label>
                           								 <input type="radio" checked="checked" name="rewardType" value="0" ${reward.rewardType eq 'card' ? 'checked="checked"' :''}> 刮刮卡
                        							</label>
                        							&nbsp;&nbsp;
                        							<label>
                            							<input  type="radio" name="rewardType" value="1" ${reward.rewardType eq 'rock' ? 'checked="checked"' :''}> 摇一摇
                       								</label>   
                    								</div>
													--%>
													<div class="form-group" id="con_findKeys">
																	...
																	<label>回复关键词</label>
																	<input type="text" class="form-control" id="findKeys" name="keys" value="${reward.key }">
																	<input type="hidden"  id="findKey" value="${reward.key }">
																	</div>
																	<div class="col-md-3">
																	<span id="tip_findKeys" for="findKeys" class="help-block " style=""></span>
															</div>
													</div>
													<input type="hidden" id="hid_replyContent" name="hid_replyContent" value="${reward.replyContent }" />
													<div class="form-group" id="con_content">
														<label for="title" class="control-label">兑奖说明<span class="required">*</span></label>
														<textarea class="form-control" rows="3" id="content" name="content">${reward.explain2.content}</textarea>
														<span class="help-block">
														</span>
													</div>
													<%-- 
													<div class="form-group" id="con_replyContent">
														...
														<input type="text" class="form-control" id="replyContent" name="replyContent" value="${reward.replyContent }">
													</div>--%>
													<input type="hidden" id="rewardId" name="rewardId" value="${reward.id }">
												</div>
											</form>
									</div>
								</div>
